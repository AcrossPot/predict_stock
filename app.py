...
stock_name = st.text_input("例：AAPL, FB, SFTBY(大文字・小文字どちらも可能)", "AAPL")

#ティッカーシンボルが不明な場合、別サイトを参照するリンク
...
try:
    #現状カレントディレクトリ上、cmdで、streamlit run app.pyでブラウザ確認が可能
    df_stock = yf.download(stock_name,'2021-01-05')

    df_stock.columns = df_stock.columns.droplevel(1)#追記
    ...
    #予測をグラフ化=====================================
        ...
        df_stock['Close'].plot(figsize=(15,6), color="green")#水色になる
        df_stock['Predict'].plot(figsize=(15,6), color="orange")#青色になる
